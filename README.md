...
本项目基于[adminMongo](https://github.com/mrvautin/adminMongo)进行优化

修改点：
1. 修复`backup.js`在部分情况下备份失败的问题。
2. 如果你只想使用DB备份恢复功能，可直接使用`/mongo`文件夹下的代码，已完成备份恢复功能封装。
3. 支持备份文件夹名称更改，你可以在`database.js`中操作，默认添加时间格式的后缀：`let dbDir = db_name + '-' + formatTime(new Date(), 'yyyyMMddhhmmss');`
4. 支持对备份后的文件夹进行压缩，如果你不需要可以注释掉压缩代码
